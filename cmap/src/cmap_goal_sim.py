...
class GoalPublisher(Node):
    def __init__(self):
        ...
        # Create a timer with a 1 second interval and a callback function
        self.experiment_list = []
        with open('./cmap/results/exp/experiment_list.txt', 'r') as f:
            for line in f:
                self.experiment_list.append(line[:-1])
        self.timer = self.create_timer(0.3, self.timer_callback)

    def timer_callback(self):
        msg = String()
        msg.data = self.experiment_list.pop(0)
