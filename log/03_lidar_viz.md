...
class LiDARSubscriber(Node) :
    ...
    def lidar_cb(self, msg) :
        ...
        #self.lidar = np.ones(self.lidar.size) * 12
        self.viz = np.zeros((self.size, self.size, 3))*255
        # x coordinate
        x_c = np.cos(self.inc*self.ind)*self.lidar*self.size/self.m/2+self.size/2
        y_c = -np.sin(self.inc*self.ind)*self.lidar*self.size/self.m/2+self.size/2
        for i in range(self.length):
            cv2.circle(self.viz,
                (int(x_c[i]),int(y_c[i])),
                int(self.res/20),
                self.cm(self.lidar[i]),-1)
